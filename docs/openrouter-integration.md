...
## Overview
This application uses **OpenRouter API** to get real-time pricing for 400+ multimodal AI models from 60+ providers.

## Why OpenRouter?

✅ **Unified pricing** - One endpoint for all providers (OpenAI, Anthropic, Google, Meta, Mistral, etc.)
✅ **Multimodal support** - Text, image, audio, and video models
✅ **Real-time pricing** - Always up-to-date from providers
✅ **Free access** - No authentication required for pricing data
✅ **400+ models** - Comprehensive coverage
...
**No API key required** for reading model pricing data.
...
```json
{
  "data": [
    ...
    },
    {
      ...
      "canonical_slug": "arcee-ai/afm-4.5b",
    ...
    },
    {
      ...
      "canonical_slug": "thedrummer/anubis-70b-v1.1",
    ...
    },
    {
      ...
      "canonical_slug": "allenai/molmo-7b-d-0924",
      "hugging_face_id": "allenai/Molmo-7B-D-0924",
...
|-------|------|---------|-------|
| `prompt` | USD per token | `"0.0000025"` | Input tokens ($2.5/1M) |
| `completion` | USD per token | "0.00001"` | Output tokens ($10/1M) |
| `image` | USD per image | `"0.01275"` | Image input cost |
| `request` | USD per request | `"0"` | Fixed per-request cost |

All values are strings. `"0"` = free.

## Model Categories

Models are automatically categorized based on modalities:

- **text**: Pure text models (GPT-4, Claude, Gemini)
- **multimodal**: Text + image input (GPT-4V, Claude 3.5 Vision)
- **image**: Text → image generation (DALL-E, Midjourney, SDXL)
- **audio**: Audio/voice models (TTS, Whisper)
- **video**: Video models (limited support)
...
- **Normalization**: Converts OpenRouter format to app format
- **Filtering**: Filter by category, provider, free models
- **Cost calculation**: Built-in cost calculator
...
// Get text models
const textModels = openRouterService.getModelsByCategory('text');
...
console.log(`Total: $${cost.totalUSD} (Rp ${cost.totalIDR})`);
```

## Normalized Model Format

The service converts OpenRouter models to a normalized format:
...
try {
  ...
  // Service returns cached data if available
  // Otherwise throws error
}
```

## Migration from ArtificialAnalysis

**Before (ArtificialAnalysis):**
- Only LLM pricing available
- No image/audio/video pricing
- Separate endpoints for media

**After (OpenRouter):**
- All modalities in one endpoint
- Real pricing for all model types
- Unified structure
- More providers

## Best Practices

1. **Cache properly**: Don't fetch on every request
2. **Handle errors**: Service has built-in fallback to cache
3. **Filter wisely**: Use category filters to reduce dropdown size
4. **Update regularly**: 1-hour cache is reasonable for pricing data

## API Documentation

Full documentation: https://openrouter.ai/docs/models
